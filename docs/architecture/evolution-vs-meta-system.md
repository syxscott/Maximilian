...
2. **Single responsibility.** `evolution` answers _"which model is best
   for this role?"_; `meta-system` answers _"should we add or remove
   this capability?"_. Two different decision scopes, two different
   packages.
...
| Have both packages compute quality deltas                       | `evolution` measures quality; `meta-system` _predicts_ deltas via `SimulationEngine` and verifies with `TruthAudit` |
