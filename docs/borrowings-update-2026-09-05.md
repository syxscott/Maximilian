---
## 审计更正(2026-09-06,源码级对照)

对下表 12 项借鉴做上游源码逐项对照后,更正三处出处/描述问题:

1. **#1 的"形状契约 + lint"实际借自 hermes** `c240e65399`(tools/skill_linter.py,incident-log-shape 与 references-sprawl 规则、60 文件上限同源),并非 oh-my-claudecode;oh-my-claudecode 只贡献了 sealed-files(validate.sh)部分。外层调研报告中"skillify 三问准入"与"hook 分级超时(30s/5s)"两条经验在上游与本项目代码中均不存在,属调研误记,未落地、不应作为依据引用。
2. **#2 反思器与上游的差距**:hermes `background_review.py` 的核心是 LLM fork 重放会话并继承父 runtime 打同一个 prefix cache;Maximilian 生产接线用的是无 LLM 的启发式 reflector(defaultReflector),该机制未移植——要复刻需给 facade 传入 LLM reflector。
3. **#10 的归属更正**:variant-runner 的 patience 早停与 combined 评分是 Maximilian 自有设计(wshobson-agents 无 `optimize()`,EvoAgentBench 无 `costDeltaUSD` 排名维度);tier 的 frontier/standard/economy 词汇亦为自创(wshobson 用 opus/sonnet/haiku 且在生成期映射到具体模型——Maximilian 的 tier 目前仅注解、不参与选择)。
