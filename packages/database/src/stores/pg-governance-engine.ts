...
export class PgGovernanceEngine {
  ...
  constructor(db: PostgresJsDatabase, defaultConfig: GovernanceConfigRow = DEFAULT_GOV_CONFIG) {
  ...
  checkProposal(input: { proposal: { action: string }; score: { riskPenalty: number } }): {
    status: "approved" | "blocked" | "pending_human"
    reason: string
  } {
  ...
  check(input: {
    ...
    allowed: boolean
    reason: string
    currentCounts: { agents: number; capabilities: number; depth: number }
    status: string
  } {
