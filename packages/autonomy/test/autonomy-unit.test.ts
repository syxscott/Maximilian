...
describe("5.1 — ExecutionStore", () => {
  ...
  it("filters by workspace, role, blueprint", async () => {
    await store.save(
      makeExecution({ id: "a", workspaceId: "ws-1", agentRole: "frontend", blueprintId: "bp-1" }),
    )
    await store.save(
      makeExecution({ id: "b", workspaceId: "ws-1", agentRole: "backend", blueprintId: "bp-2" }),
    )
    await store.save(
      makeExecution({ id: "c", workspaceId: "ws-2", agentRole: "frontend", blueprintId: "bp-1" }),
    )
...
describe("5.3 — FailurePatternAnalyzer", () => {
  ...
  it("groups failures by pattern and counts frequency", async () => {
    await store.save(
      makeExecution({
        id: "e1",
        review: makeReview({ taskId: "e1", failurePatterns: ["no_code_blocks", "truncation"] }),
      }),
    )
    await store.save(
      makeExecution({
        id: "e2",
        review: makeReview({ taskId: "e2", failurePatterns: ["no_code_blocks"] }),
      }),
    )
    await store.save(
      makeExecution({
        id: "e3",
        review: makeReview({ taskId: "e3", failurePatterns: ["truncation"] }),
      }),
    )
  ...
  it("sorts by frequency descending", async () => {
    ...
    await store.save(
      makeExecution({ id: "e2", review: makeReview({ failurePatterns: ["b", "a"] }) }),
    )
    await store.save(
      makeExecution({ id: "e3", review: makeReview({ failurePatterns: ["a", "b"] }) }),
    )
    const result = await analyzer.analyze(store)
  ...
  it("computes leaderboard insight (worst roles/models)", async () => {
    await store.save(
      makeExecution({ id: "e1", agentRole: "frontend", review: makeReview({ score: 9 }) }),
    )
    await store.save(
      makeExecution({ id: "e2", agentRole: "backend", review: makeReview({ score: 3 }) }),
    )
    await store.save(
      makeExecution({ id: "e3", agentRole: "backend", review: makeReview({ score: 2 }) }),
    )
...
describe("5.4 — EvolutionPlanner", () => {
  ...
  it("returns null when avg score and acceptance are both healthy", () => {
    const execs = Array.from({ length: 20 }, () =>
      makeExecution({
  ...
  it("persists and lists plans", async () => {
    const execs = Array.from({ length: 20 }, () =>
      makeExecution({ review: makeReview({ score: 3 }) }),
    )
    const plan = planner.plan({
      role: "frontend",
      currentVersion: "v1",
...
describe("5.6 — PromotionEngine", () => {
  ...
  it("rejects when score gain below threshold", async () => {
    const candidate = await makeCandidate("frontend", "v2")
    const currentRuns = Array.from({ length: 25 }, (_, i) =>
      makeExecution({ id: `c${i}`, blueprintId: "current-bp", review: makeReview({ score: 7 }) }),
    )
    const candidateRuns = Array.from({ length: 25 }, (_, i) =>
      makeExecution({ id: `n${i}`, blueprintId: candidate.id, review: makeReview({ score: 7.5 }) }),
    )
    const decision = await promo.decide(candidate, "current-bp", [...currentRuns, ...candidateRuns])
  ...
  it("appends to promotion-history.json", async () => {
    const candidate = await makeCandidate("frontend", "v2")
    const currentRuns = Array.from({ length: 25 }, (_, i) =>
      makeExecution({ id: `c${i}`, blueprintId: "current-bp", review: makeReview({ score: 5 }) }),
    )
    const candidateRuns = Array.from({ length: 25 }, (_, i) =>
      makeExecution({
        id: `n${i}`,
  ...
  it("uses DEFAULT_PROMOTION_CONFIG defaults (sample=20, score=10%, accept=15%)", () => {
    ...
    expect(DEFAULT_PROMOTION_CONFIG.minScoreGain).toBe(0.1)
...
describe("5.6 — PromotionEngine (status side effects)", () => {
  ...
  it("marks candidate status as 'rejected' on failure", async () => {
    ...
    const currentRuns = Array.from({ length: 25 }, (_, i) =>
      makeExecution({ id: `c${i}`, blueprintId: "current-bp", review: makeReview({ score: 8 }) }),
    )
    const candidateRuns = Array.from({ length: 25 }, (_, i) =>
      makeExecution({ id: `n${i}`, blueprintId: candidate.id, review: makeReview({ score: 8 }) }),
    )
...
describe("5.8 — AutonomyOrchestrator.observe()", () => {
  ...
  it("persists an execution record for every task in the workspace", async () => {
    const workspace = makeWorkspace([makeTask("frontend", "t1"), makeTask("backend", "t2")])
  ...
  it("skips review role tasks", async () => {
    const workspace = makeWorkspace([makeTask("frontend", "t1"), makeTask("reviewer", "t2")])
    await orchestrator.observe(workspace)
    const all = await store.listAll()
  ...
  function makeWorkspace(tasks: Task[]): Workspace {
    return {
      ...
      results: tasks.map((t) => ({
        ...
        durationMs: 100,
        metadata: {
          blueprintId: `bp-${t.agentRole}-v1`,
          blueprintVersion: "v1",
          provider: "mock",
          model: "mock-1",
          artifacts: [`${t.agentRole}-artifact`],
        },
      })),
