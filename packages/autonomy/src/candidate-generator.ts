...
export class CandidateGenerator {
  ...
  async generate(plan: EvolutionPlan, parent: AgentBlueprint): Promise<CandidateVersion> {
  ...
  async setStatus(id: string, status: CandidateVersion["status"]): Promise<CandidateVersion> {
    const existing = await this.get(id)
    if (!existing) throw new Error(`Candidate ${id} not found`)
    const updated: CandidateVersion = {
      ...existing,
