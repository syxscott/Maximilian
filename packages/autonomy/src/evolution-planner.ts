...
export class EvolutionPlanner {
  ...
  plan(input: PlannerInput): EvolutionPlan | null {
    ...
    const accepted = input.executions.map((e) => e.userFeedback?.length > 0).filter(Boolean).length
    const acceptance = input.executions.length > 0 ? accepted / input.executions.length : 0

    const needsEvolution =
      avgScore < this.config.scoreThreshold || acceptance < this.config.acceptanceThreshold
