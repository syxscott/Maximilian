...
export class PromotionEngine {
  ...
  async loadHistory(): Promise<PromotionRecord[]> {
    try {
      ...
      this.history = (JSON.parse(raw) as unknown[]).map((x) => PromotionRecordSchema.parse(x))
      return this.history
    } catch (err) {
  ...
  async decide(
    ...
    const currentRuns = allExecutions.filter((e) => e.blueprintId === currentBlueprintId)
    const candidateRuns = allExecutions.filter((e) => e.blueprintId === candidate.id)
    ...
    if (!passesScore)
      reasonParts.push(
        `score gain ${(scoreGain * 100).toFixed(1)}% < ${this.config.minScoreGain * 100}%`,
      )
    if (!passesAccept)
      reasonParts.push(
        `acceptance gain ${(acceptGain * 100).toFixed(1)}% < ${this.config.minAcceptanceGain * 100}%`,
      )
