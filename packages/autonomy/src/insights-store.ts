...
export class FailurePatternAnalyzer {
   ...
   */
  async analyze(
    executions: ExecutionStore | ExecutionRecord[],
    lookback = 50,
    ...
    for (const [pattern, group] of byPattern.entries()) {
      const scores = group.map((g) => g.review?.score ?? 0).filter((s) => s > 0)
      const avgScore = scores.length > 0 ? scores.reduce((a, s) => a + s, 0) / scores.length : 0
      const firstSeen = group.map((g) => g.startedAt).sort()[0]!
      ...
      out.push(
        FailureInsightSchema.parse({
          ...
          providers: Array.from(
            new Set(group.map((g) => g.modelAssignment?.provider).filter((p): p is string => !!p)),
          ),
          models: Array.from(
            new Set(group.map((g) => g.modelAssignment?.model).filter((m): m is string => !!m)),
          ),
          avgScore,
  ...
  async leaderboardInsight(
    ...
    const worstRoles = Array.from(byRole.entries())
      .map(([role, group]) => ({
        role,
        avgScore: avg(
          group.map((g) => g.review?.score).filter((s): s is number => s !== undefined),
        ),
