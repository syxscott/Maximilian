...
function generateFeedback(scores: ScholarEvalScore): string {
  ...
  for (const [dim, weight] of Object.entries(WEIGHTS) as [keyof ScholarEvalScore, number][]) {
    if (scores[dim] < 0.6) {
      issues.push(
        `${dim} (${(weight * 100).toFixed(0)}% weight) is low (${(scores[dim] * 10).toFixed(1)}/10)`,
      )
    }
  }
...
export function scholarEval(
  ...
  const scores: ScholarEvalScore = {
    ...
    rigor: s.rigor ?? (output.hasTests ? 0.8 : 0) + (output.buildPasses ? 0.2 : 0),
    // Impact: scope/completeness
    impact: s.impact ?? 0.5 + (output.linesOfCode ? Math.min(0.3, output.linesOfCode / 1000) : 0),
    // Novelty: non-redundant
    novelty: s.novelty ?? (output.repeatsPrior ? 0.3 : 0.7),
    // Reproducibility: build/test pass
    reproducibility:
      s.reproducibility ??
      (output.buildPasses && output.testPasses
        ? 1.0
        : output.buildPasses
          ? 0.6
          : output.hasBuild
    ...
    ethics:
      s.ethics ??
      Math.max(0, 1 - (output.securityFlags ?? 0) * 0.2 - (output.ethicalFlags ?? 0) * 0.3),
  }
