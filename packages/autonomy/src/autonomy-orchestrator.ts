...
/** Read the tenant stamp the worker places in workspace.metadata before enqueueing. */
function readWorkspaceTenantId(workspace: Workspace): string | undefined {
  const tenantId = (workspace.metadata as { tenantId?: unknown } | undefined)?.tenantId
...
export class AutonomyOrchestrator {
    ...
    const roleResults = await Promise.all(
      uniqueRoles(executions)
        ...
        .map(async (role) => {
          ...
          const roleReviews = reviews.filter((r) => roleExecs.some((e) => e.taskId === r.taskId))
  ...
  private async buildExecutionRecord(
    ...
    return {
      id,
      // Stamp the workspace's tenant so tenant-scoped store queries
      // (PgExecutionStore filters `eq(executions.tenantId, …)`) actually
      // see this record. The worker seeds workspace.metadata.tenantId
      // before enqueueing the job; records without it stay NULL-tenant
      // and are only visible to unscoped queries.
      tenantId: readWorkspaceTenantId(workspace),
      taskId: task.id,
