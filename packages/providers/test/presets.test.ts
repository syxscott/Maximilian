...
const VALID_FORMATS: ApiFormat[] = ["openai_chat", "anthropic", "gemini_native", "openai_responses"]
  ...
  it("no envKey collides between presets", () => {
    ...
    for (const p of PROVIDER_PRESETS) {
      ...
      if (existing) {
        ...
        console.warn(`envKey collision: ${p.envKey} used by ${existing} and ${p.id}`)
      }
  ...
  it("hidden presets always have a justification", () => {
    for (const p of PROVIDER_PRESETS) {
      if (p.hidden) {
        ...
        expect(justified, `${p.id} is hidden without requiresOAuth or cloudProvider`).toBe(true)
      }
    }
  })
...
describe("VISIBLE_PROVIDER_PRESETS", () => {
  ...
  it("is shorter than the full list", () => {
    expect(VISIBLE_PROVIDER_PRESETS.length).toBeLessThan(PROVIDER_PRESETS.length)
  })
})
