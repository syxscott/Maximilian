...
export function withCircuitBreaker(
  ...
  const { failureThreshold = 5, resetTimeout = 30_000, jitter = true } = options ?? {}
    ...
    await run(() => {
      if (state === "open") {
        ...
        if (elapsed >= currentResetWindow) {
          ...
          throw new Error(
            `[${provider.id}] circuit breaker is OPEN — provider unavailable (retry in ${remaining}s)`,
          )
        }
      }
      ...
      if (state === "half-open") {
        if (probeInFlight) {
          throw new Error(
            `[${provider.id}] circuit breaker is HALF-OPEN — probe in flight, retry shortly`,
          )
        }
        probeInFlight = true
      }
