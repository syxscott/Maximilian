...
import type { ChatChunk, ChatMessage, ChatOptions, ChatResponse, Provider } from "../base.js"
...
export class GeminiNativeProvider implements Provider {
  ...
  constructor(config: GeminiNativeConfig) {
    if (!config.apiKey) {
      throw new Error(`GeminiNativeProvider(${config.id}): apiKey is required`)
    }
    this.id = config.id
        ...
        try {
          ...
          if (usage) {
            ...
            if (usage.candidatesTokenCount !== undefined) {
              span?.setAttribute("llm.completionTokens", usage.candidatesTokenCount)
            }
            if (usage.cachedContentTokenCount) {
              span?.setAttribute("llm.cacheReadTokens", usage.cachedContentTokenCount)
            }
          }
          return {
            content: text,
  ...
  async *stream(messages: ChatMessage[], options: ChatOptions = {}): AsyncIterable<ChatChunk> {
    ...
    if (!response.ok || !response.body) {
      throw new ProviderError(this.id, response.status, `Gemini stream HTTP ${response.status}`)
    }
