...
export class FileWorkspaceStore {
  ...
  async saveWorkspace(workspace: Workspace, tenantId?: string): Promise<void> {
    ...
    // Write the workspace first via unstorage (keeps the path layout
    // consistent for readers), then atomically write the tenant key
    // directly to disk. The previous two-step `setItem` could land
    // `ws/{id}` first, crash, and leave the workspace with no tenant
    // record — turning a tenant-owned workspace into a tenant-less
    // one the load side would then refuse to surface. With the
    // tenant write going through writeFileAtomic (write-tmp + rename),
    // a crash mid-flight leaves the old tenant key in place instead
    // of a missing or half-written one.
    ...
    if (this.rootDir !== undefined) {
      ...
      // Non-fs driver (memory, s3, etc.) — fall back to setItem.
  ...
  async readArtifact(
    ...
    const val = await this.storage.getItem<string>(
      `artifact/${workspaceId}/${filename}`
    );
