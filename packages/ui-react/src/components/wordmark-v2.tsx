...
export function WordmarkV2({ className, ...rest }: WordmarkV2Props) {
  const reactId = useId()
  const filter = `wm-filter-${reactId.replace(/:/g, "")}`
  const mask = `wm-mask-${reactId.replace(/:/g, "")}`
  const maskGradient = `wm-mask-grad-${reactId.replace(/:/g, "")}`
