...
const Icon: React.FC<{ name: string; size?: "small" | "normal"; style?: React.CSSProperties }> = ({
  name,
  size = "normal",
  style,
}) => (
...
>(({ icon, variant = "ghost", size = "normal", className, children, ...rest }, ref) => {
  return (
    <button
      ...
      data-size={size}
      className={cn(
        "inline-flex items-center justify-center rounded p-1 hover:bg-background-stronger",
        className,
      )}
      {...rest}
    >
...
const DiffChanges: React.FC<{ changes: { additions?: number; deletions?: number } | unknown }> = ({
  changes,
}) => {
  ...
  return (
    <span
      ...
      className="inline-flex items-center gap-1 text-12-regular text-text-weak"
    >
      <span data-slot="additions" className="text-syntax-success">
        +{c?.additions ?? 0}
      </span>
      <span data-slot="deletions" className="text-syntax-error">
        −{c?.deletions ?? 0}
      </span>
    </span>
  )
      ...
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Portal>
          <DropdownMenu.Content
            sideOffset={4}
            align="end"
            className="rounded-md border border-border-weak-base bg-background-base p-1 shadow-md"
          >
            <DropdownMenu.Item
              onSelect={onEdit}
              className="cursor-pointer rounded px-2 py-1 text-13-regular hover:bg-background-stronger"
            >
              {labels.editLabel}
            </DropdownMenu.Item>
            <DropdownMenu.Item
  ...
  return (
    <div data-component="session-review" className={cn("flex h-full flex-col", props.className)}>
      <div
        ...
        className={cn(
          "flex items-center justify-between border-b border-border-weak-base p-3",
          ...
          {hasDiffs && props.onDiffStyleChange && (
            ...
            >
              <RadioGroup.Item
                value="unified"
                className={cn(
                  "rounded px-2 py-1 text-12-regular",
                  diffStyle === "unified" && "bg-background-stronger",
                )}
              >
                Unified
              </RadioGroup.Item>
              <RadioGroup.Item
                ...
                className={cn(
                  "rounded px-2 py-1 text-12-regular",
                  diffStyle === "split" && "bg-background-stronger",
                )}
              >
                Split
              </RadioGroup.Item>
          ...
          {!hasDiffs ? (
            ...
            <div className="pb-6">
              <Accordion.Root
                ...
                value={open}
                onValueChange={(value) =>
                  handleChange(Array.isArray(value) ? value : value ? [value] : [])
                }
              >
                {files.map((file) => {
                  ...
                  const tooLarge =
                    expanded && !store.force[file] && changedLines > MAX_DIFF_CHANGED_LINES
                              ...
                              >
                                ...
                                <div data-slot="session-review-file-name-container">
                                  ...
                                  <span
                                    ...
                                    className="text-13-medium text-text-base"
                                  >
                                    {getFilename(file)}
                                  </span>
                                </div>
                              ...
                              >
                                {isAdded ? (
                                  <span
                                    ...
                                    data-type="added"
                                    className="text-12-regular text-syntax-success"
                                  >
                                    Added
                                  </span>
                                ) : isDeleted ? (
                                  <span
                                    data-slot="session-review-change"
                                    data-type="removed"
                                    className="text-12-regular text-syntax-error"
                                  >
                                    Removed
                                  </span>
                                ) : (
                          ...
                          <div data-slot="session-review-diff-wrapper" className="p-2">
                            {tooLarge ? (
                              <div
                                ...
                                className="rounded-lg border border-border-weak-base bg-background-base p-6 text-center"
                              >
                                <div
                                  data-slot="session-review-large-diff-title"
                                  className="text-14-semibold text-text-base"
                                >
                                  Large diff
                                </div>
                                <div
                                  ...
                                  className="text-12-regular text-text-weak"
                                >
                                  Limit {MAX_DIFF_CHANGED_LINES.toLocaleString()}, current{" "}
                                  {changedLines.toLocaleString()}
                                </div>
                                <div
                              ...
                              React.createElement(Component, {
                                ...
                                selectedLines:
                                  store.selection?.file === file ? store.selection.range : null,
                                commentedLines,
                                media: {
                                  mode: "auto",
                                  path: file,
