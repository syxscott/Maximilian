...
export type IconV2Name = keyof typeof ICONS_V2 | (string & {})

const SPRITE_ID = "maximilian-v2-icon-sprite"
const symbol = (name: string) => `maximilian-v2-icon-${name}`
...
function ensureSprite() {
  ...
  svg.innerHTML = Object.entries(ICONS_V2)
    .map(
      ([name, icon]) =>
...
export interface IconV2Props extends Omit<SVGProps<SVGSVGElement>, "name"> {
  name: IconV2Name
  size?: "small" | "normal" | "large"
}

export function IconV2({ name, size = "normal", className, ...rest }: IconV2Props) {
  const [ready, setReady] = useState(spriteInserted)
  ...
  const iconName = (name in ICONS_V2 ? name : "plus") as keyof typeof ICONS_V2
  const icon = ICONS_V2[iconName]
  const pixelSize = size === "small" ? 14 : size === "large" ? 20 : 16
  ...
  return (
    ...
    >
      <use href={`#${symbol(iconName as string)}`} />
    </svg>
  )
}