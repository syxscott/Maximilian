...
function renderMathInText(text: string): string {
  ...
  result = result.replace(displayMathRegex, (_: string, math: string) => {
    try {
  ...
  result = result.replace(inlineMathRegex, (_: string, math: string) => {
    try {
      return katex.renderToString(math, {
        displayMode: false,
        throwOnError: false,
...
async function highlightCodeBlocks(html: string): Promise<string> {
  ...
  for (const match of matches) {
    ...
    const code = (escapedCode as string)
      .replace(/&lt;/g, "<")
      .replace(/&gt;/g, ">")
      .replace(/&amp;/g, "&")
      .replace(/&quot;/g, '"')
...
const { use: useMarked, provider: MarkedProviderBase } = createSimpleContext<MarkedParser, {
  ...
  init: (props) => {
    ...
    return jsParser as unknown as MarkedParser
  },
})

export interface MarkedProviderProps {
  nativeParser?: NativeMarkdownParser
  children?: React.ReactNode
}

export function MarkedProvider(props: MarkedProviderProps) {
  return MarkedProviderBase({
    nativeParser: props.nativeParser,
