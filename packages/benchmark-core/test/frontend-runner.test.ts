...
describe("FrontendRunner", () => {
  ...
  it("detects semantic HTML elements", () => {
    ...
    `
    const result = runner.validateComponent(code, reactContext)
    const semanticFinding = result.structuralFindings.find(
      (f) => f.label === "Uses semantic HTML elements",
    )
  ...
  it("detects aria attributes", () => {
    ...
    `
    const result = runner.validateComponent(code, reactContext)
    const ariaFinding = result.structuralFindings.find(
      (f) => f.label === "Has accessibility attributes (aria-* or role)",
    )
