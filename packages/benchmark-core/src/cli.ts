...
async function loadTasks(domain: CliArgs["domain"]): Promise<BenchmarkTask[]> {
  ...
  const domains = domain === "all" ? ["database", "devops", "frontend"] : [domain]

  for (const d of domains) {
    try {
      const mod = await import(`../../../benchmarks/${d}/tasks.js`)
      const exported = Object.values(mod).find((v) => Array.isArray(v)) as
      ...
      console.error(
        `Failed to load tasks for domain '${d}':`,
...
function printSummary(
  baseline: BenchmarkSuiteResult | null,
  maximilian: BenchmarkSuiteResult | null,
): void {
  console.log(`\n${BOLD}═══ Summary ═══${RESET}\n`)
  ...
  if (baseline) {
    ...
    console.log(
      `${BOLD}Baseline:${RESET}  pass=${GREEN}${(m.passRate * 100).toFixed(0)}%${RESET}  quality=${color(m.avgQuality)}  latency=${m.avgLatencyMs.toFixed(0)}ms  tokens=${m.totalTokens}  acceptance=${color(m.avgAcceptance)}`,
    )
  }
  ...
  if (maximilian) {
    ...
    console.log(
      `${BOLD}Maximilian:${RESET} pass=${GREEN}${(m.passRate * 100).toFixed(0)}%${RESET}  quality=${color(m.avgQuality)}  latency=${m.avgLatencyMs.toFixed(0)}ms  tokens=${m.totalTokens}  acceptance=${color(m.avgAcceptance)}`,
    )
  }
  ...
  if (baseline && maximilian) {
    ...
    const aDelta =
      maximilian.aggregateMetrics.avgAcceptance - baseline.aggregateMetrics.avgAcceptance
    console.log(
...
async function main(): Promise<void> {
  ...
  if (!provider && args.mode !== "maximilian") {
    console.error(
      `${RED}No LLM provider configured. Set OPENAI_API_KEY / ANTHROPIC_API_KEY in .env${RESET}`,
    )
