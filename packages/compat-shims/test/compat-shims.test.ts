...
describe("drizzle.ts", () => {
  ...
  it("serialColumn throws until a pg store actually uses it", () => {
    ...
    // The shim throws unconditionally (no installed drizzle in this
    // package), so we assert against the error message pattern instead
    // of a strict equality (the major-version branch depends on the
    // installed drizzle-orm, which may be 0.x or unhandled).
    expect(() => serialColumn()).toThrow(/\[compat-shims\]/)
  })
})
...
describe("llm.ts — toProviderArgs", () => {
  ...
  it("joins multiple system messages with a blank line", () => {
    ...
    expect((args.messages as Array<{ content: string }>)[0]?.content).toBe("rule 1\n\nrule 2")
  })
