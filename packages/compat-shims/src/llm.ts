...
export function toProviderArgs(
  ...
  const system = Array.isArray(req.system) ? req.system.join("\n\n") : (req.system ?? "")
...
function buildOpenAIArgs(
  ...
  const args: Record<string, unknown> = {
    model: req.model,
    messages: [...(system ? [{ role: "system", content: system }] : []), ...req.messages],
  }
...
function buildAnthropicArgs(req: NormalizedChatRequest, system: string): Record<string, unknown> {
  const args: Record<string, unknown> = {
    model: req.model,
