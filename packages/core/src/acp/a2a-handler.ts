...
export class A2AHandler {
  ...
  private preScreen(msg: AcpA2AMessage): {
  ...
  private async handleSendResp(
    ...
    try {
      const out = await recipient.receiver(msg.params.from, partsToPayload(msg.params.content))
      return {
        jsonrpc: "2.0",
        id: msg.id,
  ...
  private async handleToolInvoke(
    ...
    if (!toolPayload) {
      return {
        ...
        error: {
          ...
          message: "agent/tool/invoke requires a data part with { toolName, args }",
        },
      }
    }
    ...
    const toolImpl = (
      recipient.metadata?.tools as
        Record<string, (args: unknown) => Promise<unknown> | unknown> | undefined
    ...
    if (!toolImpl) {
      return {
        ...
        error: {
          ...
          message: `tool "${toolPayload.toolName}" not found on agent "${to}"`,
        },
      }
    }
