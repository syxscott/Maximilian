...
export const defaultAtomize: AtomizeFn = (node, maxDepth) => {
  if (shouldForceExecute({ ...node, maxDepth })) {
    return {
      ...
      reason: `depth ${node.depth} >= maxDepth ${maxDepth}, force execute`,
    }
  }
...
export function atomizeTask(opts: AtomizeTaskOpts): TaskNode {
  ...
  try {
    ...
    const taskType =
      decision.nodeType === NodeType.PLAN ? TaskType.STRATEGY : TaskType.CODE_GENERATION
 ...
 */
export function buildSubTasks(parent: TaskNode, specs: ReadonlyArray<SubTaskSpec>): BuildDagResult {
...
export function attachSubTasks(
  ...
  try {
    updatedParent = transition(
      updatedParent,
      TaskStatus.PLAN_DONE,
      `attached ${children.length} sub-tasks`,
    )
  } catch {
...
export function aggregateResults(
  ...
  try {
    return transition(
      ...
      `aggregated ${ordered.length} child results`,
    ) as TaskNode
  } catch {
    return parent
  }
