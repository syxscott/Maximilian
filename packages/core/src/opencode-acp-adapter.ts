...
export class OpencodeAcpAdapter {
  ...
  translateSend(msg: AcpA2AMessage): AcpSendTranslation {
    if (msg.method !== "agent/send") {
      throw new Error(`OpencodeAcpAdapter: only agent/send is supported (got ${msg.method})`)
    }
    ...
    for (const part of content.parts) {
      ...
      if (part.kind === "data") {
        ...
        if (v && typeof v.uri === "string" && typeof v.mime === "string") {
  ...
  async runSend(
    ...
    const taskId = msg.params.taskId ?? msg.params.messageId ?? msg.id ?? null
    if (!taskId) {
      throw new Error(
...
function idleStatusToState(status: { message?: string } | undefined): A2ATaskState {
