...
export function sanitizeDisplayLabel(raw: unknown, options: SanitizeLabelOptions = {}): string {
  ...
  const sliced = trimmed.length > maxLength ? trimmed.slice(0, maxLength) : trimmed
