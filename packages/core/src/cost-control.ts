...
export class QuotaExceededError extends Error {
  constructor(
    ...
    super(
      `tenant ${tenantId} exceeded monthly quota: $${consumedUsd.toFixed(4)} > $${monthlyCapUsd.toFixed(4)}`,
    )
  ...
  snapshot(): Array<{ tenantId: string; consumedUsd: number; remainingUsd: number }> {
    ...
    return [...this.ledgers.entries()].map(([tenantId, ledger]) => ({
      ...
      remainingUsd: Math.max(
        0,
        this.monthlyCapUsd - (ledger.monthKey === key ? ledger.consumedUsd : 0),
      ),
    }))
  }
}
...
export class RateLimitTracker {
  ...
  record429(provider: string): number {
    ...
    const backoff = Math.min(
      this.maxBackoffMs,
      this.defaultBackoffMs * Math.pow(2, state.consecutive429s - 1),
    )
  ...
  getState(
    provider: string,
  ): { consecutive429s: number; backoffUntil: number; lastSeenAt: number } | undefined {
