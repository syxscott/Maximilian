...
describe("Borrowed — RecursivePhaseRunner", () => {
  ...
  beforeEach(() => {
    ...
    bus.subscribe((e) => events.push(e), {
      types: ["task:start", "task:decomposed", "task:executed", "task:failed"],
    })
  })
  ...
  it("aggregates child results deterministically", async () => {
    const root = createTaskNode({
      id: "root",
      description:
        "This is a very long and complex process that must be broken into three separate phases. First phase is setup. Second is execution. Third is verification.",
      maxDepth: 2,
    })
    const runner = new RecursivePhaseRunner({
      workspaceId: "ws-1",
