...
describe("PermissionAuditLog", () => {
  ...
  it("caps results at the requested limit, newest first", () => {
    ...
    for (let i = 0; i < 10; i++) {
      log.record(
        mkEntry({ at: `2026-01-01T00:00:${i.toString().padStart(2, "0")}Z`, requestId: `r${i}` }),
      )
    }
  ...
  it("filters by tool and workspaceId", () => {
    const log = new PermissionAuditLog(100)
    log.record(
      mkEntry({ at: "2026-01-01T00:00:00Z", requestId: "r1", tool: "bash", workspaceId: "ws-1" }),
    )
    log.record(
      mkEntry({ at: "2026-01-01T00:00:01Z", requestId: "r2", tool: "write", workspaceId: "ws-1" }),
    )
    log.record(
      mkEntry({ at: "2026-01-01T00:00:02Z", requestId: "r3", tool: "bash", workspaceId: "ws-2" }),
    )
  ...
  it("clamps limit to MAX_LIMIT (1000)", () => {
    ...
    for (let i = 0; i < 1500; i++) {
      log.record(
        mkEntry({
          at: `2026-01-01T00:00:${(i % 60).toString().padStart(2, "0")}Z`,
          requestId: `r${i}`,
        }),
      )
    }
