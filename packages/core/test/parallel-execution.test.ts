...
describe("Concurrent task execution", () => {
  ...
  it("diamond dependency graph [A→B, A→C, B+C→D] executes in correct order", async () => {
    ...
    const tasks: Task[] = [
      ...
      { id: "D", description: "join", agentRole: "general", dependsOn: ["B", "C"], status: "pending" },
    ];
    const workspace: Workspace = {
      id: "ws-diamond",
      userRequest: "x",
  ...
  it("skips downstream tasks when a dependency fails", async () => {
    ...
    const tasks: Task[] = [
      ...
      { id: "B", description: "downstream", agentRole: "general", dependsOn: ["A"], status: "pending" },
    ];
    const workspace: Workspace = {
      id: "ws-skip",
      userRequest: "x",
    ...
    // The runtime fails the workspace on the first task failure, so
    // B is never scheduled. The plan records A's failure on the task
    // object itself; B is left in "pending" state (never started).
