...
describe("TeamOrchestrator", () => {
  ...
  it("delegates task from team A to team B successfully", async () => {
    const orch = new TeamOrchestrator({ teams: [teamA, teamB], emitEvents: true }, registry, bus)
  ...
  it("falls back to secondary team when primary fails", async () => {
    ...
    const orch = new TeamOrchestrator({ teams: [teamA, teamB, teamC] }, registryNoB, bus)
    const result = await orch.delegate({
      id: "fallback-test",
      fromTeamId: "a",
      toTeamId: "b",
  ...
  it("findCapableTeam matches task to team with matching capabilities", () => {
    const orch = new TeamOrchestrator({ teams: [teamA, teamB, teamC] }, registry, bus)
  ...
  it("compactMemory reduces older facts while keeping recent ones", async () => {
    ...
    const orch = new TeamOrchestrator({ teams: [teamA], compactKeepRecent: 3 }, registry, bus)
    ...
    const factLines = ctx.split("\n").filter((l) => l.startsWith("- ") && l.includes("fact")).length
  ...
  it("concurrent delegation limit per team is enforced", async () => {
    ...
    const orch = new TeamOrchestrator({ teams: [teamA, teamB, teamC] }, blockingRegistry, bus)
  ...
  it("emits events in correct order (created → complete | failed → escalation)", async () => {
    ...
    const orch = new TeamOrchestrator({ teams: [teamA, teamB, teamC] }, r, trackingBus)
    const result = await orch.delegate({
      id: "evt-order",
      fromTeamId: "a",
      toTeamId: "b",
