...
describe("OpencodeTeamBridge", () => {
  ...
  it("applyLifecycle emits a PATCH that mirrors AgentService.update", async () => {
    ...
    const payload = await bridge.applyLifecycle(team, "leader-b", "promote", "active", {
  ...
  it("binds to a TeamOrchestrator and translates delegation events", async () => {
    ...
    expect(bridge.listDeletes().map((d: AgentDeletePayload) => d.agentId)).toContain("leader-y")
  })
  ...
  it("rejects empty baseUrl", () => {
    expect(() => new OpencodeTeamBridge({ baseUrl: "" })).toThrow(/baseUrl/)
  })
