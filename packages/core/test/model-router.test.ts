...
describe("ModelRouter", () => {
  describe("selectModel", () => {
    ...
    // 借鉴 opencode - ModelStatus: deprecated 模型被跳过
    it("skips deprecated models (借鉴 opencode - ModelStatus)", () => {
      const router = new ModelRouter([
        {
          provider: "anthropic",
    ...
    it("keeps alpha/beta models in the routing pool (借鉴 opencode)", () => {
      const router = new ModelRouter([
        {
          provider: "anthropic",
      ...
      expect(sel.model).toBe("claude-experimental")
    })

    it("falls back to default when ALL profiles are deprecated (借鉴 opencode)", () => {
      const router = new ModelRouter([
        {
          provider: "anthropic",
          model: "claude-3-haiku-20240307",
      ...
      expect(sel.model).toBe("claude-3-haiku-20240307") // 硬编码 fallback
    })
  })
...
describe("Runtime ModelRouter integration", () => {
  ...
  it("selects cheap model for simple tasks", async () => {
    ...
    await rt.execute(
      makeWorkspace("ws-mr-2", [{ id: "t1", role: "general", desc: "Fix typo in readme" }]),
    )
  ...
  it("does not set model override when modelRouter is absent", async () => {
    ...
    await rt.execute(
      makeWorkspace("ws-mr-3", [{ id: "t1", role: "general", desc: "Fix typo in readme" }]),
    )
