...
import type { AcpA2AMessage, AcpEvent } from "../src/acp/index.js"
...
describe("OpencodeAcpAdapter", () => {
  ...
  it("translateSend lifts file-like data parts and folds other data into text", () => {
    ...
    expect(translation.textParts).toEqual([{ type: "text", text: `[data] {"foo":"bar"}` }])
  })
  ...
  it("mapSessionStatus handles session.updated / session.created gracefully", () => {
    ...
    ;(adapter as unknown as { latestState: Map<string, string> }).latestState.set(sess, "working")
  ...
  it("mapMessagesToStatuses emits one completed status per assistant message", () => {
    ...
    const messages = [
      ...
      },
      {
        ...
        model: { id: "m", providerID: "p" },
        parts: [
          { id: "p", messageID: "m2", sessionID: sess, type: "text" as const, text: "hi back" },
        ],
        cost: 0,
        tokens: { input: 0, output: 0, reasoning: 0, cache: { read: 0, write: 0 } },
  ...
  it("requires a client at construction", () => {
    expect(
      () => new OpencodeAcpAdapter({ client: undefined as unknown as OpencodeHttpClient }),
    ).toThrow(/client/)
  })
  ...
  it("preserves translation shape for snapshot-style consumers", () => {
    ...
    expect(Object.keys(translation).sort()).toEqual(
      ["agent", "contextId", "fileParts", "messageId", "textParts", "title"].sort(),
    )
