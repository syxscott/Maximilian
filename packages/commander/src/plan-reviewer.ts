...
export function reviewPlan(
  ...
  const minDimKey =
    Object.keys(scores).find((k) => scores[k as keyof PlanReviewScore] === minDimScore) ?? ""
  if (minDimScore < MIN_EACH_SCORE) {
    reasons.push(
      `Dimension "${minDimKey}" scored ${minDimScore}, minimum required is ${MIN_EACH_SCORE}`,
    )
  }
  ...
  if (countTaskTypes(tasks) < MIN_TASK_TYPES)
    suggestions.push(`Diversify task types (minimum ${MIN_TASK_TYPES} types)`)
...
export function revisePlan(
  ...
  if (revisedTasks.length < MIN_TASKS) {
    ...
    while (revisedTasks.length < MIN_TASKS && loops < MAX_REVISE_LOOPS * MIN_TASKS) {
      revisedTasks.push({
        ...revisedTasks[0],
        description: "Refined: " + (revisedTasks[0].description ?? ""),
      })
      loops++
    }
  }
