...
export async function getSession(client: OpencodeHttpClient, sessionID: string): Promise<Session> {
  const res = await client.get<{ data: Session } | Session>(messagePath(sessionID))
  return unwrapData(res)
}

/** GET /api/session → list sessions. */
export async function listSessions(client: OpencodeHttpClient): Promise<Session[]> {
  const res = await client.get<SessionListResponse | Session[]>("/api/session")
  if (Array.isArray(res)) return res
  return res.data
}

/** DELETE /api/session/{id} → delete a session. */
export async function deleteSession(client: OpencodeHttpClient, sessionID: string): Promise<void> {
...
export async function compactSession(client: OpencodeHttpClient, sessionID: string): Promise<void> {
  await client.post<void>(messagePath(sessionID, "/compact"), {})
}

/** POST /api/session/{id}/abort → abort the running session loop. */
export async function abortSession(client: OpencodeHttpClient, sessionID: string): Promise<void> {
...
export async function waitSession(client: OpencodeHttpClient, sessionID: string): Promise<void> {
