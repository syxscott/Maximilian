...
export declare interface EventBridge {
   ...
   */
  on(event: "mapped", listener: (info: MappedEventInfo) => void): this
...
export class EventBridge extends EventEmitter {
  ...
  private async runLoop(rootSignal: AbortSignal): Promise<void> {
    while (!rootSignal.aborted) {
      ...
      if (!this.shouldReconnect()) {
        throw new Error(`EventBridge: max reconnects (${this.maxReconnects}) exceeded; giving up`)
      }
  ...
  private acquireStreamSignal(parent: AbortSignal): AbortSignal {
    ...
    if (parent.aborted) {
      ...
      child.addEventListener("abort", () => parent.removeEventListener("abort", onParentAbort), {
        once: true,
      })
    }
    return child
