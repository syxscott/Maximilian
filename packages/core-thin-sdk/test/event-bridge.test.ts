    ...
    [Symbol.asyncIterator]() {
      return {
        next: (): Promise<IteratorResult<OpencodeEvent>> => {
          ...
          if ((endReason === "error" && errorDelivered) || (endReason === "error" && !rejecter)) {
...
const sampleSessionEvent = (type: string, data: Record<string, unknown>): OpencodeEvent => ({
  id: "evt_1",
  type,
  data,
})
...
describe("OPENCODE_EVENT_MAP", () => {
  ...
  it("falls back to unknown:* for unmapped events", () => {
    const draft = mapOpencodeEvent({ type: "some.future.event", data: {} }, index, "ws-1")
...
describe("EventBridge", () => {
  ...
  function buildBridge(opts: ConstructorParameters<typeof EventBridge>[0]): EventBridge {
  ...
  it("dispatches mapped events into the EventStore", async () => {
    ...
    expect(events.map((e) => e.type)).toEqual(["message:delta", "message:delta", "session:idle"])
  ...
  it("falls back to workspaceId when neither location nor sessionID is set", async () => {
    ...
    expect(store.getEvents("ws-fallback").map((e) => e.type)).toContain("server:connected")
  ...
  it("drops events when the buffer reaches its high watermark", async () => {
    ...
    await waitFor(() => bridge.getMetrics().eventsDropped >= 3, 500)
  ...
  it("reconnects with exponential backoff when the stream errors", async () => {
    ...
    pumps[1].push(sampleSessionEvent("session.idle", { sessionID: "ses_recover" }))
  ...
  it("reconnects when no events arrive within the heartbeat window", async () => {
    ...
    await waitFor(() => errors.some((e) => /heartbeat/i.test(e.message)), 500)
