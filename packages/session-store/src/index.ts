...
export class SessionStore {
  ...
  /** Current schema_version recorded in the `meta` table (0 if unset). */
  get schemaVersion(): number {
    const row = this.getMeta("schema_version")
    const version = row === null ? Number.NaN : Number.parseInt(row, 10)
  ...
  getSession(id: string): SessionRow | null {
    ...
    ).get(id) as { id: string; workspace_id: string | null; title: string | null; created_at: string | null; updated_at: string | null } | undefined
    if (!row) return null
    return {
      id: row.id,
  ...
  enqueueSteering(input: EnqueueSteeringInput): boolean {
    const result = this.statement(
      ...
      `INSERT OR IGNORE INTO steering_queue (workspace_id, text, source, receipt_id, created_at, consumed_at)
       VALUES (?, ?, ?, ?, ?, NULL)`,
    ).run(input.workspaceId ?? null, input.text, input.source ?? null, input.receiptId, input.createdAt ?? nowIso())
   ...
   */
  upsertLessonEfficacy(role: string, bucket: string, injectedCount: number, deltaSum: number): void {
    this.statement(
  ...
  getLessonEfficacy(role: string): LessonEfficacyRow[] {
    ...
    ).all(role) as Array<{ role: string; bucket: string; injected_count: number; delta_sum: number }>
    return rows.map((row) => ({
      role: row.role,
      bucket: row.bucket,
  ...
  listSessions(workspaceId?: string, limit = 100): SessionRow[] {
    ...
    ) as Array<{ id: string; workspace_id: string | null; title: string | null; created_at: string | null; updated_at: string | null }>
    return rows.map((row) => ({
      id: row.id,
      workspaceId: row.workspace_id,
      title: row.title,
