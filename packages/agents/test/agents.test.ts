...
import { BackendAgent, FrontendAgent, ReviewAgent, defaultAgentFactory } from "../src/index.js"
...
describe("BackendAgent", () => {
  it("returns a Result with the provider content and metadata", async () => {
    ...
    const result = await agent.execute(makeTask("task-1", "Build a TODO API"), makeContext())
...
describe("FrontendAgent", () => {
  it("returns a frontend Result with provider content", async () => {
    ...
    const result = await agent.execute(makeTask("task-2", "Build a TODO UI"), makeContext())
...
describe("ReviewAgent", () => {
  it("parses well-formed JSON and clamps the score into [0, 10]", async () => {
    ...
    const review = result.metadata.review as {
      score: number
      issues: string[]
      suggestions: string[]
      summary: string
  ...
  it("throws when the model produces no JSON at all", async () => {
    ...
    const agent = new ReviewAgent(provider)
    await expect(agent.execute(makeTask("task-r", "review"), makeContext())).rejects.toThrow(
      /valid JSON/i,
    )
  })
  ...
  it("uses prior results in the user prompt", async () => {
    ...
    const agent = new ReviewAgent(provider)
    await agent.execute(makeTask("task-r", "review"), makeContext([priorResult]))
