...
export class PendingProposalStore {
  ...
  async save(input: {
    ...
    await fs.writeFile(this.filePath(input.proposal.id), JSON.stringify(pending, null, 2), "utf-8")
  ...
  async resolve(
    ...
    await fs.writeFile(this.filePath(proposalId), JSON.stringify(resolved, null, 2), "utf-8")
