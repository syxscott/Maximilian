...
export class SimulationEngine {
  ...
  async compare(
    ...
    const scoreA =
      resA.estimatedAvgQuality -
      resA.totalEstimatedCost * 0.01 -
      resA.totalEstimatedLatencyMs * 0.0001
    const scoreB =
      resB.estimatedAvgQuality -
      resB.totalEstimatedCost * 0.01 -
      resB.totalEstimatedLatencyMs * 0.0001
  ...
  async simulateDelta(before: SimulationInput, after: SimulationInput): Promise<SimulationDelta> {
    ...
    return SimulationDeltaSchema.parse({
      ...
      latencyDeltaMs: round2(
        afterResult.totalEstimatedLatencyMs - beforeResult.totalEstimatedLatencyMs,
      ),
