...
import { CapabilityRecordSchema, type CapabilityRecord, type CapabilityStatus } from "./types.js"
...
export class CapabilityRegistry {
  ...
  async propose(input: {
    ...
    if (existing) {
      throw new Error(
        `Capability ${input.capabilityId} already exists in status ${existing.status}`,
      )
    }
  ...
  async transition(id: string, to: CapabilityStatus): Promise<CapabilityRecord> {
