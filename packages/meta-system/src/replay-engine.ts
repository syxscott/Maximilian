...
import { ReplayOutcomeSchema, type ReplayOutcome, type Proposal } from "./types.js"
...
export interface ReplayDeps {
  ...
  captureSimulation?: (
    proposal: Proposal,
  ) => Promise<{ before: SimulationInput; after: SimulationInput }>
}
...
export class ReplayEngine {
  ...
  async replay(input: ReplayInput): Promise<ReplayOutcome> {
    ...
    const scored = affected.map((e) => e.review?.score).filter((s): s is number => s !== undefined)
