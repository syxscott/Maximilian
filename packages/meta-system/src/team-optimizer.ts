...
import { TeamOptimizerHintSchema, TEAM_OPTIMIZER_CONFIG, type TeamOptimizerHint } from "./types.js"
...
export class TeamOptimizer {
  ...
  async suggest(input: OptimizerInput): Promise<TeamOptimizerHint> {
    ...
    const hasReview = input.graph.nodes.some((n) => n.role === "review" || n.role === "reviewer")
    if (!hasReview) {
      suggestions.push({
    ...
    const estimatedCost =
      input.graph.nodes.reduce((sum, n) => {
...
export async function persistHint(hint: TeamOptimizerHint, rootDir: string): Promise<void> {
