...
export class MetaOrchestrator {
  ...
  async cycle(input: MetaCycleInput): Promise<MetaCycleResult> {
    ...
    const discovery = await this.deps.discovery.discover(
      input.discoverySignals,
      knownIds
    );
    ...
    for (const p of discovery.proposals) {
      try {
        ...
        await this.deps.orgMemory.record(
          "capability_proposed",
          p.capabilityId,
          { proposalId: p.id, source: p.source }
        );
      } catch (err) {
      ...
      for (const c of reloadedCapabilities) {
        ...
        if (current.status === "experimental") {
          ...
          if (projected.length >= maxCaps) {
            blockedBy.push(`maxCapabilities: ${projected.length}/${maxCaps} reached, further promotions blocked`);
            break;
          }
    ...
    if (!blockedBy.some((r) => r.includes("maxAgents"))) {
      for (const c of activated) {
        if (birthBudget >= maxAgents) {
          blockedBy.push(`maxAgents: ${birthBudget}/${maxAgents} reached, ${activated.length - births.length} pending births skipped`);
          break;
        }
    ...
    const activeBlueprintIds = input.blueprints
    ...
    const teamHint = await this.deps.teamOptimizer.suggest({
      graph: input.graphs[0] ?? { id: "g-empty", nodes: [], edges: [], capabilities: [], createdAt: new Date().toISOString(), status: "draft" },
   ...
   */
  private async runProposal(proposal: Proposal, onApplied?: () => Promise<void>): Promise<Phase8ProposalTrace> {
    ...
    try {
      ...
      return {
        ...
        score: { proposalId: proposal.id, qualityGain: 0, latencyPenalty: 0, costPenalty: 0, riskPenalty: 0, utility: 0, approved: false, reason: `pipeline error: ${String(err)}` },
        rollout: undefined,
