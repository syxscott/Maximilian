...
describe("MetaSystemOpencodeBridge", () => {
  ...
  it("M9: registers a plugin capability only for the explicit session it was scoped to", async () => {
    ...
    expect(bridge.getTeamState("team-seeded")?.pluginCapabilities ?? []).not.toContain("scoped")
...
describe("OpencodeDigitalTwin", () => {
  ...
  it("simulate rejects empty teamId / scenario", async () => {
    ...
    await expect(twin.simulate({ teamId: "", scenario: "x", maxSteps: 1 })).rejects.toThrow(
      /teamId/,
    )
    await expect(twin.simulate({ teamId: "t", scenario: "", maxSteps: 1 })).rejects.toThrow(
      /scenario/,
    )
  })
