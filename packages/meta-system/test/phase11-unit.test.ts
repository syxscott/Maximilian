...
describe("PendingProposalStore", () => {
  ...
  it("resolve() throws on already-resolved proposal", async () => {
    ...
    await expect(store.resolve(proposal.id, "approved", "alice", "changed mind")).rejects.toThrow(
      "already resolved",
    )
  })

  it("resolve() throws on nonexistent proposal", async () => {
    await expect(store.resolve("nonexistent", "approved", "alice", "nope")).rejects.toThrow(
      "not found",
    )
  })
...
describe("Phase 11 — HITL gate in MetaOrchestrator", () => {
  ...
  it("retire proposal is paused by HITL gate, not applied", async () => {
    ...
    const manualRetireBlueprint = async (id: string) => {
      retiredId = id
    }
