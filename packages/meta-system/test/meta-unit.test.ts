...
describe("6.2 CapabilityRegistry", () => {
  ...
  it("rejects duplicate propose", async () => {
    await registry.propose({ capabilityId: "test_cap", displayName: "Test" })
    await expect(
      registry.propose({ capabilityId: "test_cap", displayName: "Test" }),
    ).rejects.toThrow()
  })
...
describe("6.3 MetaAgent", () => {
  ...
  it("emits delete for each retirement decision", () => {
    ...
    expect(plan.decisions.some((d) => d.action === "delete" && d.agentRole === "bad_agent")).toBe(
      true,
    )
  })
...
describe("6.4 TeamOptimizer", () => {
  ...
  it("suggests grow_team for low avg quality", async () => {
    const executions = Array.from({ length: 5 }, () => makeExecution({ review: makeReview(4) }))
    const hint = await optimizer.suggest({
      graph: makeTeamGraph(),
      executions,
    })
...
describe("6.9 GovernanceEngine", () => {
  ...
  it("ignores retired blueprints in agent count", () => {
    ...
    const v = limitedEngine.check({
      graphs: [],
      capabilities: [],
      blueprints: [makeBlueprint({ retiredAt: new Date().toISOString() }), makeBlueprint()],
    })
    expect(v.allowed).toBe(true)
  })
...
describe("6.X MetaOrchestrator", () => {
  ...
  it("Phase 6-7 path: rejects merge decisions with DIGITAL_TWIN_ENABLED blockedBy", async () => {
    // MetaAgent emits a merge decision when two roles have low avgScore
    // and ≥ 5 usages. In Phase 6-7 (no pipeline), the orchestrator must
    // NOT actually mutate the graph — it must surface an explicit
    // blockedBy reason instead of writing a misleading orgMemory event.
    const now = new Date().toISOString()
    const recentExecutions: ExecutionRecord[] = []
    // Two low-scoring roles, each with ≥5 executions (triggers merge).
    ...
    // The MetaAgent emitted at least one merge decision...
    expect(result.changePlan.decisions.some((d) => d.action === "merge")).toBe(true)
    // ...but the Phase 6-7 path surfaces a blockedBy reason rather than
    // silently recording a "successful" merge in orgMemory.
    expect(result.blockedBy.some((r) => r.includes("DIGITAL_TWIN_ENABLED"))).toBe(true)
  })
})
