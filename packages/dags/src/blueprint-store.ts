...
export class BlueprintStore {
  ...
  async save(blueprint: AgentBlueprint): Promise<void> {
    // Auto-bump updatedAt on every write so callers don't have to remember.
    // Schema rejects a stale or missing updatedAt — this keeps the invariant
    // "updatedAt >= createdAt" enforced at the persistence boundary.
    const stamped: AgentBlueprint = { ...blueprint, updatedAt: new Date().toISOString() }
    const validated = AgentBlueprintSchema.parse(stamped)
