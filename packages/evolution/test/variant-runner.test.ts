...
function makeMockExecutor(
  responses: Array<ExecuteResult | Error>,
): VariantExecutor & { calls: number } {
  let calls = 0
  return {
    get calls() {
      return calls
...
describe("VariantRunner", () => {
  ...
  it("throws when task or executor is missing", async () => {
    ...
    await expect(
      runner.runWith(exec, task as Task, { agentRole: "frontend", workspaceId: "w1" }),
    ...
    await expect(
      runner.runWith(
        ...
        null,
        task,
        { agentRole: "frontend", workspaceId: "w1" },
      ),
  ...
  it("runs the default 3 variants via identityMutator", async () => {
    ...
    expect(
      report.runs.every(
        (r) => r.parent.systemPrompt === report.parentProfile.manifest?.systemPrompt,
      ),
    ).toBe(true)
  ...
  it("falls back to winnerIndex=-1 if every variant errored", async () => {
    const exec = makeMockExecutor([new Error("a"), new Error("b")])
    const task = makeTask("t1")
    const report = await runner.runWith(exec, task, {
      agentRole: "frontend",
      workspaceId: "w1",
