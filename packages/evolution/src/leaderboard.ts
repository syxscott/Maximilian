...
export class Leaderboard {
  ...
  overall(opts: { minDomains?: number } = {}): LeaderboardEntry[] {
    ...
    for (const [, group] of byRoleModel) {
      ...
      const avgTime = group.reduce((a, e) => a + e.avgExecutionTime * e.sampleSize, 0) / totalSamples;
      const satisfaction = group.reduce((a, e) => a + e.userSatisfaction * e.sampleSize, 0) / totalSamples;
      const first = group[0]!;
      const baselineScores = group.map((e) => e.baselineScore).filter((x): x is number => x !== undefined);
      ...
      const deltaScore = deltaScores.length > 0
        ? deltaScores.reduce((a, b) => a + b, 0) / deltaScores.length
        : undefined;
      const costDeltas = group.map((e) => e.costDeltaUSD).filter((x): x is number => x !== undefined);
      const costDeltaUSD = costDeltas.length > 0
...
export function aggregate(
  ...
  const baselineByRoleModel = new Map<string, { baseline: number; decidedAt: string; decisions: EvolutionDecision[] }>();
  if (opts.decisions) {
  ...
  for (const [k, slice] of buckets.entries()) {
    ...
    const baseline = baselineByRoleModel.get(`${role}|*`) ?? baselineByRoleModel.get(`${role}|${model}`);
    ...
    const costDeltaUSD = baselineScore !== undefined
      ? avgCostUSD - (slice.length > 1
          ? slice.slice(0, Math.max(1, Math.floor(slice.length / 2)))
