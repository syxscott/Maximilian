...
export class ModelSelector {
  ...
  scoreEntry(entry: LeaderboardEntry): number {
    ...
    const costP = Math.min(entry.avgCostUSD / this.config.maxCostUSD, 1) * this.config.costWeight
    const samplePenalty = entry.sampleSize < this.config.minSamples ? 0.1 : 0
    return quality * (1 - latencyP) * (1 - costP) - samplePenalty
  }

  select(role: AgentRole, board: Leaderboard, fallback?: { provider: Provider }): ModelSelection {
...
function buildReason(role: AgentRole, entry: LeaderboardEntry, cfg: SelectorConfig): string {
  const parts: string[] = []
