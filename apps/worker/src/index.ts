...
import { AgentRuntime, type RuntimeSink, type Workspace, FileMemoryStore, type ModelRouter } from "@max/core";
...
async function main() {
  ...
  if (!config.DATABASE_URL) {
    log.fatal("DATABASE_URL is required for the worker process (file-based storage not supported for queue mode)");
    process.exit(1);
  }
  ...
  const modelSelectorPort: import("@max/core").ModelSelectorPort | undefined = modelRouting?.selector;
  ...
  runtime.on((event) => {
    if (event.type === "task-complete") {
      ...
      log.error({ taskId: event.taskId, workspaceId: event.workspaceId, error: event.error }, "task failed");
    }
  });
  ...
  const processor: WorkspaceProcessor = async (
    ...
    if (workspace.status !== "planning") {
      log.warn({ workspaceId, status: workspace.status }, "workspace not in planning state — skipping");
      return;
    }
  ...
  const worker = createWorker(redisUrl, processor, concurrency);
  ...
  worker.on("failed", (job: Job<WorkspaceJobData> | undefined, err: Error) => {
    log.error({ jobId: job?.id, workspaceId: job?.data.workspaceId, err: err.message }, "job failed");
  });
  ...
  const shutdown = async (signal: string) => {
    ...
    try {
      await worker.close();       // stop accepting new jobs, wait for in-flight
      await closeDb();            // close DB pool
