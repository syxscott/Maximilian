...
import { ClipboardProvider, ArgsProvider, KVProvider, RouteProvider, useRoute, SDKProvider, useSDK, ThemeProvider, ExitProvider, useExit, PluginRuntimeProvider, usePluginRuntime, useTheme, useToast, useDialog, } from "./context";
// ---------------------------------------------------------------------------
// Public entry point
// ---------------------------------------------------------------------------
export async function run(input) {
    ...
    try {
        const instance = render(_jsx(Root, { args: input.args, config: input.config, url: input.url, directory: input.directory, token: input.token, onExit: onExit }));
...
function Root(props) {
    return (_jsx(ExitProvider, { exit: props.onExit, children: _jsx(ReactErrorBoundary, { fallback: ({ error, reset }) => (_jsx(ErrorComponent, { error: error, reset: reset, mode: "dark" })), children: _jsx(ClipboardProvider, { children: _jsx(ArgsProvider, { value: props.args, children: _jsx(KVProvider, { children: _jsx(ToastProvider, { children: _jsx(RouteProvider, { children: _jsx(SDKProvider, { url: props.url, directory: props.directory, token: props.token, children: _jsx(ThemeProvider, { mode: "dark", children: _jsx(DialogProvider, { children: _jsx(PluginRuntimeProvider, { children: _jsx(App, { config: props.config }) }) }) }) }) }) }) }) }) }) }) }));
}
...
function App({ config }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", width: "100%", children: [ready ? (_jsxs(Box, { flexGrow: 1, flexDirection: "column", children: [routeData.type === "home" ? (_jsx(Home, {})) : routeData.type === "session" ? (_jsx(Session, { sessionID: routeData.sessionID })) : routeData.type === "plugin" ? (_jsx(PluginRoute, { id: routeData.id })) : (_jsx(Text, { children: t("tui.unknownRoute") })), _jsx(Box, { flexShrink: 0, children: _jsx(pluginRuntime.Slot, { name: "app_bottom" }) }), _jsx(pluginRuntime.Slot, { name: "app" })] })) : null, _jsx(StartupLoading, { ready: ready }), _jsx(Text, { children: " " }), void theme, void exit] }));
}
// ---------------------------------------------------------------------------
...
function Home() {
    ...
    return (_jsxs(Box, { flexDirection: "column", paddingLeft: 1, paddingRight: 1, children: [_jsx(Text, { bold: true, color: theme.theme.text, children: "Maximilian" }), loading ? (_jsxs(Text, { color: theme.theme.textMuted, children: ["Connecting to ", sdk.url, "\u2026"] })) : error ? (_jsxs(Text, { color: "red", children: ["Failed to load: ", error] })) : (_jsxs(_Fragment, { children: [_jsx(HealthRow, { health: health }), _jsx(UsageRow, { usage: usage }), _jsx(PendingRow, { pending: pending }), _jsx(ExecutionsRow, { executions: executions })] })), _jsx(Text, { color: theme.theme.textMuted, children: "Press / for language, ctrl+\\ for the command palette (stub)." })] }));
}
function HealthRow({ health }) {
    ...
    return (_jsxs(Box, { marginTop: 1, children: [_jsxs(Text, { color: ok ? "green" : "yellow", children: [ok ? "●" : "○", " "] }), _jsxs(Text, { children: ["API ", health.status, " \u00B7 providers: ", health.providers.map((p) => p.id).join(", ") || t("common.none"), " \u00B7 evolution: ", health.evolution] })] }));
}
function UsageRow({ usage }) {
    useLocale();
    if (!usage)
        return null;
    return (_jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsx(Text, { bold: true, children: t("tui.today") }), _jsxs(Text, { children: ["  requests: ", usage.totalRequests, " \u00B7 cost: $", usage.totalCostUsd.toFixed(4), " \u00B7 success: ", (usage.successRate * 100).toFixed(1), "%"] }), _jsxs(Text, { children: ["  tokens: ", usage.realTotalTokens.toLocaleString(), " (cache hit ", (usage.cacheHitRate * 100).toFixed(1), "%)"] }), usage.unpricedRequestCount > 0 && (_jsxs(Text, { color: "red", children: ["  \u26A0 ", usage.unpricedRequestCount, " unpriced request(s) \u2014 pricing table missing entry"] }))] }));
}
function PendingRow({ pending }) {
    useLocale();
    if (pending.length === 0)
        return null;
    return (_jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsxs(Text, { bold: true, color: "yellow", children: ["Pending governance (", pending.length, ")"] }), pending.slice(0, 3).map((p) => (_jsxs(Text, { children: ["  ", p.proposal.action, " ", p.proposal.subject, " \u00B7 utility ", p.score.utility.toFixed(2)] }, p.proposalId)))] }));
}
function ExecutionsRow({ executions }) {
    useLocale();
    if (executions.length === 0)
        return null;
    return (_jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsx(Text, { bold: true, children: t("tui.recentExecutions") }), executions.map((e) => (_jsxs(Text, { children: ["  [", e.status, "] ", e.userPrompt.slice(0, 60), e.userPrompt.length > 60 ? "…" : ""] }, e.id)))] }));
}
function Session({ sessionID }) {
    useLocale();
    const theme = useTheme();
    return (_jsxs(Box, { flexDirection: "column", paddingLeft: 1, paddingRight: 1, children: [_jsxs(Text, { bold: true, color: theme.theme.text, children: ["Session ", sessionID] }), _jsx(Text, { color: theme.theme.textMuted, children: t("tui.sessionRouteStub") })] }));
}
function PluginRoute({ id }) {
    ...
    return (_jsx(Box, { paddingLeft: 1, paddingRight: 1, children: _jsxs(Text, { color: "yellow", children: ["Plugin route not found: ", id] }) }));
}
