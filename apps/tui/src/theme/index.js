...
export function subscribeThemes(listener) {
...
export function upsertTheme(name, theme) {
    if (!name)
        return false;
    if (!isTheme(theme))
        return false;
...
export function tint(base, overlay, alpha) {
    const r = base.r + (overlay.r - base.r) * alpha;
    const g = base.g + (overlay.g - base.g) * alpha;
    const b = base.b + (overlay.b - base.b) * alpha;
    return RGBA.fromInts(Math.round(r * 255), Math.round(g * 255), Math.round(b * 255));
}
export function terminalMode(colors) {
...
export function generateSystem(colors, mode) {
...
export function generateSubtleSyntax(theme, overrides) {
