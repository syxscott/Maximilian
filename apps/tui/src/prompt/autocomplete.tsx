...
export const Autocomplete = forwardRef<AutocompleteRef | undefined, AutocompleteProps>(
  function Autocomplete(props, forwardedRef) {
    ...
    // Mirror `props.anchor` so the search effect can read the latest anchor
    // callback without re-running on every parent render. See the effect
    ...
    useEffect(() => {
      ...
      const t = setTimeout(() => {
        sdk.client
          .get<{ data: Array<{ path: string; type: string }> }>(
            `/fs/find?query=${encodeURIComponent(query)}&limit=20`,
          )
          .then((res) => {
            if (cancelled) return
            // Read anchor via a ref so the search effect doesn't re-run on
            // every parent render. The previous version listed the whole
            // `props` object in the deps array — since the parent rebuilds
            // `props.anchor` inline on every render, the effect re-ran
            // constantly, restarting the 80ms debounce and dropping every
            // in-flight search request via the `cancelled` cleanup.
            const width = Math.max(40, anchorRef.current().width - 4)
            setFiles(
              (res.data ?? []).map((item) => ({
