...
export type MoveSessionSelection =
  { type: "directory"; directory: string; subdirectory: boolean } | { type: "new"; name?: string }
...
export function usePromptMove(input: {
  ...
  const moveExistingSession = useCallback(
    async (sessionID: string, _selection: MoveSessionSelection): Promise<void> => {
      const session = (sync.data.session as Array<{ id: string; directory?: string }>).find(
        (s) => s.id === sessionID,
      )
      void session
  ...
  const getDirectory = useCallback(async (_context?: string): Promise<string | undefined> => {
