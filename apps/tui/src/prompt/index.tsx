...
export type PromptPart =
  ...
  | {
      ...
      filename: string
      url: string
      source?: { type: string; text?: { start: number; end: number; value: string }; path: string }
    }
...
export const Prompt = forwardRef<PromptRef | undefined, PromptProps>(
  function Prompt(props, forwardedRef) {
    const sync = useSync()
    const sdk = useSDK()
    ...
    const status = (sync.data.session_status as Record<string, { type: string }>)[
      props.sessionID ?? ""
    ] ?? { type: "idle" }
    ...
    const submit = useCallback(async (): Promise<boolean> => {
      ...
      try {
        ...
        if (mode === "shell" || text.startsWith("!")) {
          ...
          await sdk.client
            .post(`/session/${props.sessionID}/shell`, {
              command,
              agent: agent.name,
          ...
          if (!props.sessionID) return false
          await sdk.client
            .post(`/session/${props.sessionID}/command`, {
              command: command.slice(1),
              arguments: rest.join(" "),
              agent: agent.name,
              model: `${model.providerID}/${model.modelID}`,
              parts: parts.filter(
                (p): p is Extract<PromptPart, { type: "file" }> => p.type === "file",
              ),
            })
            .catch(() => undefined)
        } else {
    ...
    const commands = useMemo(() => {
      return (sync.data.command as Array<{ name: string; description?: string }>).filter(
        (c) => c.source !== "skill",
      )
    }, [sync.data.command])
