...
const BindingObject = Schema.StructWithRest(Schema.Struct({
...
const keybind = (value, description) => ({ default: value, description });
...
export const KeybindOverrides = Schema.Struct(Object.fromEntries(Object.entries(Definitions).map(([name, item]) => [
    name,
    Schema.optional(BindingValueSchema).annotate({ description: item.description }),
]))).annotate({ description: "TUI keybinding overrides" });
export const Descriptions = Object.fromEntries(Object.entries(Definitions).map(([name, item]) => [name, item.description]));
...
const CommandDescriptions = Object.fromEntries(Object.entries(Definitions).map(([name, item]) => [
    CommandMap[name] ?? name,
    item.description,
]));
export function toBindingConfig(keybinds) {
    return Object.fromEntries(Object.entries(keybinds));
}
