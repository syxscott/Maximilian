...
export const { use: useData, provider: DataProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const latestTool = (assistant, callID) =>
      assistant?.content?.findLast?.(
        (item) => item.type === "tool" && (callID === undefined || item.id === callID),
      )
    const latestText = (assistant, textID) =>
      assistant?.content?.findLast?.((item) => item.type === "text" && item.id === textID)
    const latestReasoning = (assistant, reasoningID) =>
      assistant?.content?.findLast?.((item) => item.type === "reasoning" && item.id === reasoningID)
        ...
        switch (event?.type) {
          ...
          case "session.next.tool.success":
            message.update(event.properties.sessionID, (draft) => {
              ...
              // entire React tree).
              const providerPayload = event.properties.provider ?? {}
              match.state = {
                status: "completed",
