...
export function cliErrorMessage(input) {
    ...
    if (model) {
        const suggestions = Array.isArray(model.suggestions)
            ? model.suggestions.filter((item) => typeof item === "string")
            : [];
        return [
    ...
    if (frontmatter)
        return field(frontmatter, "message") ?? "";
...
function named(input, name) {
    return isRecord(input) && (input.name === name || input._tag === name);
}
function configData(input, tag) {
    if (!isRecord(input))
        return undefined;
    if (input.name === tag && isRecord(input.data))
        return input.data;
    if (input._tag === tag)
        return input;
    return undefined;
