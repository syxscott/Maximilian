...
function state(api, item) {
    if (!item.enabled) {
        return _jsx(Text, { color: api.theme.current.textMuted, children: "disabled" });
    }
    return (_jsx(Text, { color: item.active ? api.theme.current.success : api.theme.current.error, children: item.active ? "active" : "inactive" }));
}
...
function Install(props) {
    ...
    return (_jsx(props.api.ui.DialogPrompt, { title: "Install plugin", placeholder: "npm package name", busy: busy, busyText: "Installing plugin...", description: () => (_jsxs(Box, { flexDirection: "row", gap: 1, children: [_jsx(Text, { color: props.api.theme.current.textMuted, children: "scope:" }), _jsx(Text, { color: busy ? props.api.theme.current.textMuted : props.api.theme.current.text, children: global ? "global" : "local" }), !busy && _jsx(Text, { color: props.api.theme.current.textMuted, children: "(tab toggle)" })] })), onConfirm: (raw) => {
            if (busy)
                return;
            const mod = raw.trim();
            if (!mod) {
...
function showInstall(api) {
    api.ui.dialog.replace(() => _jsx(Install, { api: api }));
}
function View(props) {
    const [terminalWidth, setTerminalWidth] = useState(80);
    const [list, setList] = useState(props.api.plugins.list());
    ...
    const rows = useMemo(() => [...list]
        ...
        .map((item) => row(props.api, item, terminalWidth)), [list, terminalWidth]);
    const flip = (x) => {
        if (lock)
            return;
    ...
    return (_jsx(DialogSelect, { title: "Plugins", options: rows, current: cur, onMove: (item) => setCur(item.value), actions: [
            {
                title: "toggle",
...
function show(api) {
    api.ui.dialog.replace(() => _jsx(View, { api: api }));
}
const tui = async (api) => {
