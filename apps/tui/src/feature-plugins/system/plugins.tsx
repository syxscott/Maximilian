...
function state(api: TuiPluginApi, item: TuiPluginStatus) {
  if (!item.enabled) {
    return <Text color={api.theme.current.textMuted}>disabled</Text>
  }

  return (
    <Text color={item.active ? api.theme.current.success : api.theme.current.error}>
    ...
    <props.api.ui.DialogPrompt
      ...
      description={() => (
        <Box flexDirection="row" gap={1}>
          <Text color={props.api.theme.current.textMuted}>scope:</Text>
          <Text color={busy ? props.api.theme.current.textMuted : props.api.theme.current.text}>
            {global ? "global" : "local"}
          </Text>
          {!busy && <Text color={props.api.theme.current.textMuted}>(tab toggle)</Text>}
        </Box>
      )}
      onConfirm={(raw) => {
        if (busy) return
