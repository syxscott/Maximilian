...
function commandText(command, shortcut) {
    if (!shortcut)
        return shortcutText(command);
    return `${shortcutText(command)} or ${shortcutText(shortcut)}`;
}
function press(shortcut, text) {
    if (!shortcut)
        return undefined;
    return `Press ${shortcutText(shortcut)} ${text}`;
}
function configShortcut(api, command) {
...
export function Tips(props) {
    ...
    return (_jsxs(Box, { flexDirection: "row", maxWidth: "100%", children: [_jsx(Text, { color: theme.warning, flexShrink: 0, children: '● Tip ' }), _jsx(Text, { flexShrink: 1, wrap: "word", children: parts.map((part, index) => (_jsx(Text, { color: part.highlight ? theme.text : theme.textMuted, children: part.text }, index))) })] }));
}
const TIPS = [
    ...
    (shortcuts) => shortcuts.sessionQuickSwitch1() && shortcuts.sessionQuickSwitch9()
    ...
    (shortcuts) => shortcuts.messagesPageUp() && shortcuts.messagesPageDown()
    ...
    (shortcuts) => shortcuts.commandList()
...
const INPUT_UNDO_TIP = (shortcuts) => press(shortcuts.inputUndo(), "to undo changes in your prompt");
const TERMINAL_SUSPEND_TIP = (shortcuts) => press(shortcuts.terminalSuspend(), "to suspend the terminal and return to your shell");
