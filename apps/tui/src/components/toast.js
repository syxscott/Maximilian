...
export function ToastProvider({ children }) {
  ...
  const value = React.useMemo(() => ({ currentToast, show, error }), [currentToast, show, error])
  React.useEffect(() => {
    return () => clearTimer()
  }, [clearTimer])
  return _jsxs(ToastContext.Provider, {
    value: value,
    children: [children, _jsx(Toast, { current: currentToast })],
  })
}
  ...
  return _jsxs(Box, {
    ...
    children: [
      current.title
        ...
        : null,
      _jsx(Text, { color: DEFAULT_COLORS.text, wrap: "wrap", children: current.message }),
    ],
  })
}
