...
export function Sidebar(props) {
  ...
  const session = useMemo(
    () => sync.data.session.find((s) => s.id === props.sessionID),
  ...
  return _jsxs(Box, {
    ...
    children: [
      _jsxs(Box, {
        ...
        paddingRight: 1,
        children: [
          _jsx(Slot, { name: "sidebar_title" }),
          _jsxs(Box, {
            paddingRight: 1,
            flexDirection: "column",
            children: [
              _jsx(Text, { bold: true, color: theme.text, children: session.title ?? "Untitled" }),
              session.workspaceID
                ? _jsx(Text, {
                    dimColor: true,
                    children: workspace
                      ? _jsx(WorkspaceLabel, {
                          ...
                          name: workspace.id,
                          status: project.workspace.status?.(workspace.id) ?? "error",
                          icon: true,
                        })
                      : _jsx(WorkspaceLabel, {
                          type: "unknown",
                ...
                : null,
              session.share?.url
                ? _jsx(Text, { dimColor: true, children: session.share.url })
                : null,
            ],
          }),
          _jsx(Slot, { name: "sidebar_content" }),
        ],
      }),
      _jsxs(Box, {
        flexShrink: 0,
        ...
        children: [
          ...
          _jsxs(Text, {
            ...
            children: [
              ...
              _jsx(Text, { bold: true, children: "Code" }),
            ],
          }),
        ],
      }),
