...
export const CommandPaletteDialog = forwardRef(function CommandPaletteDialog(_props, ref) {
    ...
    const options = useMemo(() => visibleCommands.map((command) => ({
        label: command.title,
        value: command.name,
    })), [visibleCommands]);
    ...
    if (!open)
        return null;
    if (options.length === 0) {
        return (_jsx(Box, { flexDirection: "column", paddingLeft: 1, paddingRight: 1, children: _jsx(Text, { children: t("tui.noCommandsRegistered") }) }));
    }
    return (_jsxs(Box, { flexDirection: "column", paddingLeft: 1, paddingRight: 1, children: [_jsx(Box, { marginBottom: 1, children: _jsx(Text, { bold: true, children: t("tui.commands") }) }), _jsx(SelectInput, { items: options, onSelect: (item) => {
                    const cmd = visibleCommands.find((c) => c.name === item.value);
                    setOpen(false);
                    cmd?.onSelect();
                } }), _jsx(Box, { marginTop: 1, children: visibleCommands.find((c) => c.suggested) ? (_jsx(Text, { dimColor: true, children: "suggested commands are highlighted first" })) : null })] }));
});
