...
export type OpencodeSessionStatus = "idle" | "busy" | "retry" | "error" | "compacting" | "unknown"
...
export class OpencodeStateStore extends EventEmitter {
  ...
  applyEvent(event: StoredEvent): OpencodeSessionState | undefined {
    ...
    const updated: OpencodeSessionState = {
      ...
      messageCount: isMessageEvent(event.type) ? session.messageCount + 1 : session.messageCount,
      toolCallCount: isToolEvent(event.type) ? session.toolCallCount + 1 : session.toolCallCount,
      lastEventAt: event.timestamp,
      lastEventType: event.type,
...
function pushBounded<T>(arr: ReadonlyArray<T>, item: T, max: number): ReadonlyArray<T> {
