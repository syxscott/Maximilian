...
export function govRoutes(deps: GovRouteDeps) {
  ...
  return {
    ...
    resolveProposal: async (c: Context) => {
      ...
      const { action, reason, user } = c.req.valid("json" as never) as {
      ...
      if (action === "reject") {
        ...
        await orgMemory
          .record("proposal_rejected_by_human", pending.proposal.subject, {
            proposalId: id,
      ...
      await orgMemory
        .record("proposal_approved_by_human", pending.proposal.subject, {
          proposalId: id,
          action: pending.proposal.action,
          applied: true,
          resolvedBy: user,
...
async function executeMutation(
  ...
  switch (action) {
    ...
    case "birth": {
      ...
      if (!cap)
        throw new MutationError("capability_not_found", `Capability "${subject}" not found.`)
