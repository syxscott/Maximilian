...
function hasExactPricingEntry(provider: string, model: string): boolean {
  const exact = DEFAULT_PRICING.find(
    (p) => p.provider === provider && p.model === model,
  );
  if (exact) return true;
  const providerWildcard = DEFAULT_PRICING.find(
    (p) => p.provider === provider && p.model === "*",
  );
...
export const usageSummaryRoute = createRoute({
  ...
  responses: {
    ...
    503: { content: { "application/json": { schema: ErrorSchema } }, description: "Evolution disabled" },
