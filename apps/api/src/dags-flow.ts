...
/**
 * Build a workspace + plan from a DAGS composed team.
 */
export async function buildDagsWorkspace(
  dags: DAGS,
  userRequest: string
): Promise<{ workspace: Workspace; plan: Plan }> {
...
export async function runDagsFlow(
  ...
  try {
    ...
    const factory = deps.dags.buildAgentFactory(composed);
    ...
    if (deps.telemetry) {
      executionTrace = await deps.telemetry.recordExecution({
        ...
        assignedTeamGraph: {
          ...
          nodes: composed.graph.nodes.map((n) => ({
            id: n.id,
            role: n.role,
