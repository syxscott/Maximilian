...
function rec(overrides: Partial<MetricRecord> & { timestamp: string; provider?: string; model?: string; tokenInput: number; tokenOutput: number }): MetricRecord {
  return {
...
describe("aggregateUsageSummary", () => {
  ...
  it("computes success rate and unpriced flag", () => {
    const records = [
      rec({ timestamp: ISO_INSIDE, tokenInput: 100, tokenOutput: 50, provider: "anthropic", model: "claude-sonnet-4-5" }),
      rec({ timestamp: ISO_INSIDE, tokenInput: 100, tokenOutput: 50, provider: "anthropic", model: "claude-sonnet-4-5" }),
      rec({ timestamp: ISO_INSIDE, tokenInput: 100, tokenOutput: 50, provider: "anthropic", model: "claude-sonnet-4-5", error: "boom" }),
      // unknown provider — should round to ~$0 and flag unpriced
      rec({ timestamp: ISO_INSIDE, tokenInput: 100, tokenOutput: 50, provider: "unknown-provider", model: "mystery" }),
    ];
    const range = resolveUsageRange("today", NOW);
    const summary = aggregateUsageSummary(records, range, "today");
