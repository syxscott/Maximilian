...
function makeTeamGraph(overrides: Partial<TeamGraph> = {}): TeamGraph {
  ...
  return {
    ...
    capabilities: ["frontend"],
    nodes: [{ id: "n1", blueprintId: "bp-1", role: "frontend", displayName: "FE", dependsOn: [] }],
    edges: [],
    ...
    const executions: ExecutionRecord[] = [
      ...
      ...Array.from({ length: 6 }, () =>
        makeExecution({
          agentRole: "low_b",
          blueprintId: "bp-b",
          review: makeReview(3.5),
          durationMs: 1500,
        }),
      ),
      ...Array.from({ length: 6 }, () =>
        makeExecution({
          agentRole: "slow_agent",
          blueprintId: "bp-slow",
          review: makeReview(8),
          durationMs: 90000,
        }),
    ...
    const cmp = await sim.compare(
      {
        ...
        graph,
        profiles: {
          frontend: { costPerCall: 1, latencyMs: 1000, qualityScore: 9 },
      ...
      },
      {
        ...
        graph,
        profiles: {
          frontend: { costPerCall: 5, latencyMs: 5000, qualityScore: 6 },
          backend: { costPerCall: 5, latencyMs: 5000, qualityScore: 6 },
        },
      },
    )
