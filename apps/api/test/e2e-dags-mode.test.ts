...
describe("E2E: DAGS_MODE=true /api/chat + autonomy loop", () => {
  ...
  beforeEach(async () => {
    ...
    const approvalRuntimes = new Set<{
      resolveApproval(
        requestId: string,
  ...
  it("runs DAGS compose + autonomy observe() and surfaces results", async () => {
    ...
    const approvalRequest = (eventLog.get(chatBody.workspaceId) ?? []).find(
      (event) => event.type === "approval-request",
    ) as Extract<RuntimeEvent, { type: "approval-request" }> | undefined
    ...
    const execBody = (await execRes.json()) as {
      total: number
      items: Array<{ agentRole: string; review?: { score: number } }>
    }
    ...
    const status = (await statusRes.json()) as {
      ...
      roles: Array<{ role: string; avgScore: number }>
    }
