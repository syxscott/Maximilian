...
describe("SseReplayBuffer", () => {
  it("assigns monotonic ids per workspace", () => {
    ...
    expect(a1.id).toBe(1);
    expect(a2.id).toBe(2);
    expect(b1.id).toBe(1); // separate counter per workspace
  });
  ...
  it("evicts oldest entries beyond capacity but keeps id counter monotonic", () => {
    ...
    // Client that saw id=2 and reconnects should get only id=3.
    const after2 = buf.since("ws-1", 2);
