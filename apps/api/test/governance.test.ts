...
async function buildOrchestrator(
  rootDir: string,
  opts?: { hitlRiskThreshold?: number; rolloutMode?: string },
) {
  ...
  const deps: MetaOrchestratorDeps = {
    ...
    manualSaveBlueprint: async (bp) => {
      savedBlueprints.push({ id: bp.id, role: bp.role })
    },
...
describe("Phase 11 — HITL Governance API", () => {
  ...
  it("human approval resolves pending proposal", async () => {
    ...
    // Approve it.
    const resolved = await pendingStore.resolve(
      proposal.id,
      "approved",
      "admin",
  ...
  it("resolve() throws on nonexistent proposal", async () => {
    const { pendingStore } = await buildOrchestrator(tmp)
    await expect(pendingStore.resolve("nonexistent", "approved", "admin", "nope")).rejects.toThrow(
      "not found",
    )
  })
