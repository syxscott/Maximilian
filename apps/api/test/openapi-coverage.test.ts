    ...
    for (const [group, routes] of Object.entries(routeGroups)) {
      for (const { method, path } of routes) {
        ...
        expect(
          ...
          `[${group}] ${method.toUpperCase()} ${path} should be registered`,
        ).toBeDefined()
      }
    }
  })
      ...
      for (const { method, path } of routes) {
        if (method === "post" || method === "put") {
          ...
          expect(
            ...
            `[${group}] ${key} should have requestBody content`,
          ).toBeDefined()
        }
      }
    }
