/**
 * SSE replay buffer tests — exercises the per-workspace IN-MEMORY ring
 * buffer (`src/lib/sse-replay.ts::SseReplayBuffer`) and `Last-Event-ID`
 * parsing semantics.
 *
 * The durable JSONL-backed replay log (`src/event-log.ts`) and its Hono
 * handler (`src/sse-replay.ts`) are tested separately in
 * `apps/api/test/sse-replay.test.ts`.
 *
