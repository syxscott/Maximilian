...
describe("permissions routes", () => {
  ...
  it("GET returns defaults when no file is persisted", async () => {
    ...
    // Fresh install returns the default deny patterns for sensitive paths
    // (.env, .ssh, .aws, etc.) - see #619: read=allow default would let
    // agents exfiltrate secrets without these deny rules.
    expect(body.patterns.read?.["**/.env"]).toBe("deny")
    expect(body.patterns.read?.["**/.ssh/**"]).toBe("deny")
    expect(body.patterns.glob?.["**/.env"]).toBe("deny")
    expect(body.patterns.grep?.["**/.ssh/**"]).toBe("deny")
  })
...
describe("permissions audit endpoint", () => {
  ...
  it("returns audit entries from the runtime with optional filters", async () => {
    ...
    const app = buildOpenApiApp(tmp, {
      getPermissionAudit: (q) => entries.filter((e) => (q?.tool ? e.tool === q.tool : true)),
  ...
  it("total reflects full filtered count, not page size", async () => {
    ...
    const app = buildOpenApiApp(tmp, {
      getPermissionAudit: (q) => {
        const filtered = entries.filter((e) => (q?.tool ? e.tool === q.tool : true))
