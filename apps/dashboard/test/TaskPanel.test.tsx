    ...
    const ws: Workspace = {
      ...
      plan: {
        ...
        tasks: [
          ...
          { id: "t2", description: "Build API", agentRole: "backend", dependsOn: ["t1"], status: "running" },
        ],
      },
    };
  ...
  it("shows dependency info", () => {
    const ws: Workspace = {
      ...
      plan: {
        ...
        tasks: [
          { id: "t1", description: "First task", agentRole: "general", dependsOn: [], status: "completed" },
          { id: "t2", description: "Second task", agentRole: "general", dependsOn: ["t1"], status: "pending" },
        ],
      },
    };
    render(<TaskPanel workspace={ws} />);
    // Match by full text content because React renders adjacent expressions
    // as separate text nodes and getByText's regex doesn't always span them.
    expect(screen.getByText((_, el) => el?.textContent === "Depends on: t1")).toBeInTheDocument();
  });
  ...
  it("shows error for failed tasks", () => {
    const ws: Workspace = {
      ...
      plan: {
        rationale: "Test",
        tasks: [
          { id: "t1", description: "Failing task", agentRole: "general", dependsOn: [], status: "failed", error: "timeout" },
        ],
      },
    };
    render(<TaskPanel workspace={ws} />);
  ...
  it("shows rationale", () => {
    const ws: Workspace = {
      ...
      plan: {
        ...
        tasks: [
          { id: "t1", description: "Task A", agentRole: "general", dependsOn: [], status: "pending" },
        ],
      },
    };
    render(<TaskPanel workspace={ws} />);
