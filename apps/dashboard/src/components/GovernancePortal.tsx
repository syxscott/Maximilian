...
export function GovernancePortal() {
  ...
  const proposalActionSchema = z.object({
    ...
    reason: z
      .string()
      .min(1, t("governance.proposals.reasonRequired"))
      .max(2000, t("governance.proposals.reasonTooLong")),
  })
  ...
  useEffect(() => {
    ...
    // Key on `id` rather than the whole `actionState` object so opening
    // the same proposal twice in a row (closing + reopening) does reset,
    // but unrelated re-renders that produce a new wrapper object don't.
    // The previous effect dep included `actionState` directly, which meant
    // react-hook-form's `reset()` ref change could trigger an extra reset
    // (clearing the user's typed reason mid-edit) without an actual modal
    // open transition.
  }, [actionState?.id, reset])
          ...
          <DialogHeader>
            <DialogTitle>
              {actionState?.action === "approve"
                ? t("governance.proposals.approveAction")
                : t("governance.proposals.rejectAction")}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              <label htmlFor="gov-user" className="block text-sm text-muted-foreground mb-1">
                {t("governance.proposals.yourName")}
              </label>
              <Input id="gov-user" {...register("user")} placeholder="admin" />
              {errors.user && (
                <p className="text-sm text-destructive mt-1">{errors.user.message}</p>
              )}
            </div>
            <div>
              <label htmlFor="gov-reason" className="block text-sm text-muted-foreground mb-1">
                {t("governance.proposals.reason")}
              </label>
              <Textarea
                id="gov-reason"
                {...register("reason")}
            ...
            {resolveMutation.isError && (
              <p className="text-sm text-destructive">
                {resolveMutation.error instanceof Error
                  ...
                  : t("governance.proposals.actionFailed")}
              </p>
            )}
          </div>
          <DialogFooter>
            ...
            >
              {resolveMutation.isPending
                ? t("common.processing")
                : actionState?.action === "approve"
                  ? t("governance.proposals.approve")
                  : t("governance.proposals.reject")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    ...
    <Card className="bg-muted/30 border-border/50">
      <CardContent className="p-5">
        ...
        <div className="flex gap-4 text-xs text-muted-foreground mb-4">
          <span>
            {t("governance.sim.cost")}: {simulation.costDelta >= 0 ? "+" : ""}
            {simulation.costDelta.toFixed(2)}
          </span>
          <span>
            {t("governance.sim.latency")}: {simulation.latencyDeltaMs >= 0 ? "+" : ""}
            {simulation.latencyDeltaMs.toFixed(0)}ms
          </span>
          <span>
            {t("governance.sim.quality")}: {simulation.qualityDelta >= 0 ? "+" : ""}
            {simulation.qualityDelta.toFixed(2)}
          </span>
          <span>
            {t("governance.sim.risk")}: {simulation.riskDelta >= 0 ? "+" : ""}
            {simulation.riskDelta.toFixed(2)}
          </span>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <Button variant="destructive" onClick={() => onAction("reject")} className="flex-1">
...
function MetricCell({
  ...
  return (
    <div className="text-center">
      <div className={`text-base font-medium ${colorClass}`}>{value.toFixed(2)}</div>
