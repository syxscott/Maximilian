...
export function usePerfTier(): PerfController {
  ...
  //   * the first reacts to `mode` only and re-runs the benchmark when the
  //     user toggles "auto" off and back on. Previously this effect also
  //     fired when `detected` flipped, which made the benchmark re-run as
  //     a side-effect of the detection it just completed.
  //   * the second applies the CSS class + `effective` whenever either
  //     `mode` (manual override) or `detected` (auto result) changes.
  useEffect(() => {
    if (mode !== "auto") return
    ...
    detectTier(ctrl.signal)
      .then((t) => {
        if (!ctrl.signal.aborted) setDetected(t)
      })
      .catch(() => {
        if (!ctrl.signal.aborted) setDetected("low")
      })
    return () => ctrl.abort()
  }, [mode, detected])

  // Apply the resolved tier (mode override OR detected value) to the DOM.
  useEffect(() => {
    const next: PerfTier = mode === "auto" ? (detected ?? "low") : mode
    setEffective(next)
    applyPerfTierClass(next)
  }, [mode, detected])
